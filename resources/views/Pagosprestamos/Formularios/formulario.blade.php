...
if($bandera==1){
	...
	$valorc = $pagosprestamos->prestamo_id;
	$valorm = $pagosprestamos->caja_id;
	$valorb = $pagosprestamos->banco_id;
}
?>
@if($se_pago)
	<p>
...
@else
		Cuotas: <b>{{$f}}</b>/{{$m->num_cuotas}}    |    Capital: <b>{{'$ '.number_format($p,2)}}</b>/{{'$ '.number_format($m->monto,2)}} |    Monto de cuota: {{'$ '.number_format($m->valor_cuotas,2)}}
