...
<div class="launcher">
  ...
  <div class="tooltip">
    <a href={!! asset('/bancomobiliarios?nombre='.$name.'&estado='.$cam) !!}>
      @if(!$cam)
...
<div class="panel">
	...
	<center>
		<table id="block">
			...
			<?php $a=1; ?>
			@foreach($bancomobiliariosAc as $banm)
			<tr>
				<td>{{$a}}</td>
				<td>{{$banm->id_Banco}}</td>
				<td>{{$banm->cantidad}}</td>
				...
				<td>
					<div class="up">
						...
						<div class="image">
							<div class="tooltip">
								<a href={!! asset('/bancomobiliarios/'.$banm->id.'/edit') !!}>
							...
							<div class="tooltip">
								@include('Bancomobiliarios.Formularios.darDeBaja')
								<span class="tooltiptextup">Papelera</span>
							</div>
							<div class="tooltip">
								<a href={!! asset("/bancomobiliarios/".$banm->id) !!}>
		 ...
		 <div id="act">
      {!! str_replace ('/?', '?', $bancomobiliariosAc->appends(Request::only(['nombre','estado']))->render ()) !!}
    </div>
	</center>
</div>
@stop